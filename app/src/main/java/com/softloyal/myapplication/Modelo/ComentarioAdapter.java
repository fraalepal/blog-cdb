...
public class ComentarioAdapter extends RecyclerView.Adapter<ComentarioAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ComentarioAdapter.ViewHolder holder, int position) {
        ...
        String commentMessage = comentarioList.get(position).getMessage();
        holder.setComment_message(commentMessage);

        String user_id = comentarioList.get(position).getUser_id();
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setComment_message(String message){
            ...
            comment_message = mView.findViewById(R.id.comment_message);//Identificamos el elemento donde irá el mensaje en el layout
            comment_message.setText(message);//Le hacemos un set para cambiar su valor

        }
        public  void setData(String name, String image){
            commentUserName = mView.findViewById(R.id.comment_username);//Identificamos el elemento donde irá el mensaje en el layout
            commentUserImage = mView.findViewById(R.id.comment_image);//Identificamos el elemento donde irá el mensaje en el layout
            commentUserName.setText(name);//Le hacemos un set para cambiar su valor

            RequestOptions placeholderOption =new RequestOptions(); //Indicamos que el lugar donde se encontrarán los comentarios está identificado por un placeholder
            placeholderOption.placeholder(R.drawable.profile_placeholder);

            Glide.with(context).applyDefaultRequestOptions(placeholderOption).load(image).into(commentUserImage);//Con Glide cambiamos la imagen del placeholder por la del usuario que comenta
